...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.incentive) {
		var incentiveData = rpcData.incentive;

		if (incentiveData.address) {
			var incentiveReward = incentiveData.amount;
			
			reward -= incentiveReward;
			rewardToPool -= incentiveReward;
			
			var incentiveScript = util.addressToScript(incentiveData.address);
			txOutputBuffers.push(Buffer.concat([
				util.packInt64LE(incentiveReward),
				util.varIntBuffer(incentiveScript.length),
				incentiveScript
			]));
		}
		
	} else if (rpcData.payee) {
